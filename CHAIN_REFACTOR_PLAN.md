...
- 当前为降低 Hodgepodge 对异步搜索的日志噪声，并行执行器已改为稳定命名的有界线程池，这属于临时降噪措施，不等同于线程模型重构
...
- 当前实现仍允许异步规划读取世界，后续若要彻底消除此类兼容告警，需要将规划读取改为快照化或主线程化
...
- [x] 为 `ChainPreviewState` 补充渲染层可用的版本标记或等价脏标记
- [x] 新增 `ChainPreviewRenderer`，在客户端渲染事件中消费预览快照并绘制
- [x] 从旧 `SpaceCalculator`/`RenderCache` 中提取可复用的网格构建与缓存逻辑
- [x] 先跑通基础线框预览，再决定是否接入旧 shader
- [x] 修正预览起点未纳入渲染的问题
...
- 客户端已恢复基础线框预览渲染，渲染层只消费 `ChainPreviewState`
