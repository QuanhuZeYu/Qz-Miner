...
- 连锁状态由 Qz-UILib 的 HUD 虚拟窗口在屏幕左上角以液态玻璃卡片统一显示（内容随状态变化经 Signal 刷新，空状态整窗隐藏）；关闭态 HUD 不挂常驻工具栏（无内容时整窗隐藏、工具栏本来看不到）；打开 GUI 时由 UILib 自动隐藏
- 打开聊天输入框后，工具栏的「编辑 HUD」按钮进入 UILib 布局编辑子模式：拖动连锁状态 HUD 预览调整位置，缩放 `- / 1:1 / +` 也在该编辑态统一提供；取消/Esc 放弃本次修改；布局为会话内存（UILib 4.9.1 不跨重启持久化）
...
- 四个范围交互子模式的服务端规划与客户端预览都使用可按 deadline 恢复的 `BoxScanTraverser`，扫描以触发点为中心、边长 `2 x radius + 1` 的完整立方范围；目标无需相邻，不匹配坐标只会被跳过，不会阻断后续空间扫描
...
- planning、客户端 preview 与非 GT 普通执行在对应 Tick stage 内共享一个单调时钟 soft deadline；`general.tickBudgetMs` 默认 `15ms`，合法范围 `1..40ms`
- deadline 只阻止下一次世界读取、Forge 回调或目标事务；已经开始的事务会完整收口，因此它不是硬实时中断，也不承诺单 Tick 绝不超过配置值
- 已删除 work-unit、空气 `1024:1` 计费、`maxBreakPerTick` 与 50ms 节流；容量上限、队列回压、watchdog 和 `chainMaxBlocks` 仍独立生效
- GT 线缆替换在完整规划与资源/规模预校验通过后仍单 tick 原子执行，显式绕过普通 deadline，避免跨 tick 混压
...
- `5.3.x` 客户端与服务端只要版本字符串完整合法，就忽略 patch、prerelease 与 build qualifier
  互通；stable、prerelease、branch/dirty dev 均适用。
- `5.0.x`、`5.1.x`、`5.2.x`、`5.10.x` 与畸形版本不会被当成 5.3；5.3 family 内的 17 个 packet ID/Side、wire
  framing、协议、ordinal/code/mask 与 20-path 配置 schema 已冻结。不兼容变更必须升级新 minor。
...
- 优化客户端预览：连锁执行期间锁定当前预览目标，并在共享 soft deadline 的安全点让出和恢复
...
- HUD 状态展示：通过 Qz-UILib 4.9.1 的 HUD 虚拟窗口（`HudWindowFactory` 契约 + 状态 Signal）显示执行阶段、模式/子模式、服务端限制与匹配数量、对象组同步、预览进度及 AREA 区域尺寸；外观为 UILib 公开材质 API 的液态玻璃卡片（`UiBackdrop.liquidGlass` + `SceneSurfaceBinder`，宿主外壳关闭），关闭态不注册常驻外接工具栏、不自绘缩放按钮；注册单点在 `ClientProxy.init`，锚点为屏幕左上角，断线不重注册。聊天工具栏「编辑 HUD」按钮（`ChatActionService` 注册，label/tooltip 走 `ClientI18n`）发布 `HudEditService.requestEdit` 意图，编辑期预览/拖动/边界夹取/提交与缩放工具全部归 UILib 编辑宿主，Miner 只注册可编辑目标与预览工厂（`HudEditTarget` 不声明 toolbarSpec）
...
- 状态职责拆分：`ChainRequest` 保存请求参数，`ChainRuntimeState` 只保留规划遍历队列与订阅句柄，执行目标和计数由 `ChainExecutionContext` 承载
...
- `tickBudgetMs`：planning、客户端 preview 与非 GT 普通执行共享的每 Tick soft deadline，默认 `15`，合法范围 `1..40`
