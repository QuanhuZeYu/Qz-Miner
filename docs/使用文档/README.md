...
- `client.objectGroups`：每个客户端玩家自己的对象组列表。配置页以中文展示已配置规则、搜索、状态选择、结果与错误反馈；每行必须有唯一非空 `id` 和至少一个成员。成员可通过方块搜索 Picker 选择全部、单个或多个 metadata，也可继续使用完整 registry 语法，例如 `minecraft:log@0`、`minecraft:log@*`、`minecraft:log@[0,4,8,12]`。再次编辑同一 registry 会用本次选择替换并归一为唯一规则，不会与旧状态做 OR；其它 registry、无法解析的 raw 和非字符串值保持原位。Picker 只枚举客户端已注册方块及其 `getSubBlocks` 实际暴露且处于 0..15 的物品子类型，不依赖世界或 NEI，也不推断未暴露的方块状态 metadata；无可见子类型时仍可选择全部（`@*`），特定 metadata 可在 raw 配置中手写。对象组匹配仍只使用 registry + metadata，不使用 NBT、TileEntity 或矿辞推断。
