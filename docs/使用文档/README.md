...
- 对象组模式使用现有连锁子模式滚轮。保存、RELOAD 或连接建立后，客户端发送完整对象组配置；HUD 的 `Confirmed` 只表示服务端已接受当前 `requestedRevision`，服务端回传的 `authoritativeRevision` 是实际权威水位。客户端忽略旧提交和旧连接的迟到结果。服务端对象组规则按玩家隔离，进行中的连锁继续使用启动时冻结的组快照。
