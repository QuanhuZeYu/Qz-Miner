...
- 对象组模式使用现有连锁子模式滚轮。保存、RELOAD 或连接建立后，客户端发送同一 `CommittedSnapshot` 中的 revision 与完整对象组配置；HUD 的 `Confirmed` 只表示服务端已接受该请求。客户端按连接与 requested revision 保存有界请求快照，迟到 ACK 或旧连接 ACK 会被忽略，确认投影使用服务端实际确认的请求规则。服务端对象组规则按玩家隔离，进行中的连锁继续使用启动时冻结的组快照。
