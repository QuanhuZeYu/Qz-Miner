...
- HUD 布局编辑：打开聊天输入框后，工具栏的「编辑 HUD」按钮进入 UILib 布局编辑子模式，拖动连锁状态 HUD 预览调整屏幕位置。拖动、边界夹取、草稿/提交/取消与 Esc 优先级全部归 UILib 编辑宿主，Miner 只声明可编辑目标与预览内容；编辑会话中的预览不跟随 HUD 显示门（否则连锁键松开时预览零尺寸、无法拖动）。放置为**会话内存**（UILib 4.9.1 尚未做跨重启持久化），退出游戏不保留。
