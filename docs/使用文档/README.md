...
- `client.objectGroups`：每个客户端玩家自己的对象组列表。每行必须有唯一非空 `id` 和至少一个成员；成员可在配置页通过方块搜索 Picker 选择全部、单个或多个 metadata，也可继续使用完整 registry 语法，例如 `minecraft:log@0`、`minecraft:log@*`、`minecraft:log@[0,4,8,12]`。Picker 只枚举客户端已注册方块及其 `getSubBlocks` 实际暴露且处于 0..15 的物品子类型，不依赖世界或 NEI，也不推断未暴露的方块状态 metadata；无可见子类型时仍可选择全部（`@*`），特定 metadata 可在 raw 配置中手写。对象组匹配仍只使用 registry + metadata，不使用 NBT、TileEntity 或矿辞推断。
