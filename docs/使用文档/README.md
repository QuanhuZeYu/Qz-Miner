...
- `client.autoToolSelection`：自动工具预选，默认关闭。启用后仅在无界面、空光标且准星指向方块的采掘类子模式中生效；按下连锁键后从背包槽 0..35 选择可采掘工具，优先保留目标所需的精准采集/时运能力，并遵守最小剩余耐久。快捷栏工具直接切槽，背包工具通过原版库存交换；松开连锁键时请求恢复。手动滚轮/数字键改槽、丢弃物品或库存事务拒绝会暂停当前按键周期，断线、世界卸载和切换世界只清理本地状态，不发送迟到恢复。
