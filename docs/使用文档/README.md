...
- `client.autoToolSwapEnabled`：是否启用自动工具换位，默认 `true`。仅在按住连锁键且当前子模式由破坏方块触发时参与；创造模式不参与。当前主手能够收获且满足耐久条件时不会换位，候选工具至少保留 2 点耐久；实际挖掘效率仍会采样，但低效率本身不再排除可收获工具。
...
- `client.autoToolPrioritySelectors`：自动工具候选的有序优先级列表，不是白名单。支持 `<namespace:path>@*`、`<namespace:path>@<meta>` 与 `ore:<name>`；先按最早命中的规则排序，同优先级及未命中的合格候选按个人库存槽位 `0..35` 排序。
  - **运行节奏**：按键上升沿建立本轮，首次可立即匹配，稳定目标之后最多每 10 tick 重匹配一次。首块成功前，准星切到不同方块或 metadata 会先恢复旧借用工具，再只为最终目标重新选择；同方块、同 metadata 的坐标变化不重选。准星短暂越过空气 1 tick 不触发库存事务，连续 2 个客户端 END tick 无有效方块才确认丢失并恢复。爆破、隧道、同块区域、区段清理和普通连锁的规划只计算世界/几何/结构候选，不会因规划开始时空手、工具损坏或背包尚无工具而提前删目标；confirmed/预览因此表示“候选”，不是“保证全部可破坏”。连锁中空手、不足 2 点耐久或当前工具不能收获队首时，服务端会在消费前按实时事实请求同 round 接替或拒绝当前目标；换入工具会再次由服务端复验。未声明 harvestTool 的未知工具使用通用 `Item.canHarvestBlock` 判定，不依赖 TiC/模组白名单。现存超时、发送、身份/库存或 ledger 等会话级 STOP 仍可能结束本次剩余执行，留待独立阶段收窄。GT 线缆 SPECIAL 不走此门。
  - **收口边界**：打开 GUI 或切换热栏槽位时会先恢复已换入工具，并保留当前轮次；松开连锁键、关闭配置或连锁自然回到空闲时会恢复后结束。断线、重生、切维度或通信异常时不会跨生命周期盲目恢复，以服务端最后同步的库存状态为准。
  - **预览刷新**：SWAP、连锁中接替和最终恢复的库存布局首次可见后，会使用首次瞄准时冻结的方块 seed 重新计算预览；首块已破坏时不会把 origin 当成空气。重复同步、断线或已停止预览不会复活旧结果。
  - **验收状态**：planner 宽进、主线程逐目标权威、poll 预算与通用 Item API 有自动化覆盖，但同版本 client/dedicated、真实爆破/隧道体验、HUD/预览、连续多次接替、空手与模组方块及完整生命周期矩阵仍待用户实机验证。
