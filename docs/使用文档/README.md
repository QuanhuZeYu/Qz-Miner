...
- `client.autoToolPrioritySelectors`：自动工具候选的有序优先级列表，不是白名单。支持 `<namespace:path>@*`、`<namespace:path>@<meta>` 与 `ore:<name>`；先按最早命中的规则排序，同优先级及未命中的合格候选按个人库存槽位 `0..35` 排序。
  - **运行节奏**：按键上升沿建立本轮，首次可立即匹配，之后最多每 10 tick 重匹配一次；首个方块成功破坏后冻结普通 SWAP。连锁中主手降到 1/0 时，服务端会在消费队首前暂停，客户端下一 tick 按服务端目标选择候选并执行同 round 接替；无候选、关闭配置、拒绝或超时会结束本次连锁，队首不会被假消费。GT 线缆 SPECIAL 不走此门。
