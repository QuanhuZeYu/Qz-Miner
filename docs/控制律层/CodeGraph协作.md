...
- 控制论五层文档（`docs/{设定值层,控制律层,传感层,反馈层,诊断层}/`）承载稳定协作记忆
...
1. 先读 `docs/README.md` 与 `AGENTS.md`，确认任务所属控制层
2. 再按任务进入 `设定值层/`、`控制律层/`、`传感层/`、`反馈层/`（含 errors）、`诊断层/`（含 reviews）
3. 涉及具体类、方法、调用链、依赖面、影响面时，优先使用 `codegraph` MCP 查询结构关系
4. 对 CodeGraph 返回的关键文件和符号，用 `Read`、`Grep`、`Glob` 或 LSP 再确认源码现状
5. 修改完成后按 `docs/控制律层/稳定命令.md` 运行定向验证，不以 CodeGraph 静态分析结果代替测试
...
- 已经应该进入 `docs/使用文档/`、`docs/反馈层/errors/`、`docs/诊断层/reviews/` 或 `docs/反馈层/决策/` 的稳定结论
- 未经源码或测试确认的推断性结论

## 写回规则

- CodeGraph 查询发现目录、入口、命令或长期边界变化时，写回 `docs/设定值层/边界.md` 或对应控制律层文档
- CodeGraph 查询支持了关键方案取舍时，写入 `docs/反馈层/决策/`
- CodeGraph 查询暴露可复现错误或审查结论时，写入 `docs/反馈层/errors/`（通则进 `错误预防.md`）或 `docs/诊断层/reviews/`
