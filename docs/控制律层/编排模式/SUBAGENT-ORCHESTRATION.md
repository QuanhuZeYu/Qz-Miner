...
1. 主 agent 通过只读侦察核实现状，必要时请 oracle 裁决，并把可执行结论写入任务单。
2. fixer 读取任务单，只改写集，执行指定测试与验证，通过后提交，并填写“结果”。写集不足时不得写盘，返回 `INCOMPLETE` 并指出缺少的路径。
3. reviewer 读取同一任务单与 Git diff，独立核对目标、范围、验收、验证和 `NORTH_STAR.md` I1-I10，按 P0/P1/P2 中文输出。
...
- 结论先列 findings，并用 `file:line` 证明；P0 是阻断性缺陷，P1 是重要缺陷，P2 是非阻断观察
- P0/P1 必须说明具体失败行为，个人偏好和无具体风险的覆盖扩张不得作为阻断项
- 关键 API、路径、行号和不变量断言由主 agent 抽检，不照搬单点结论
- 多个 agent 结论冲突时追加只读盘查，不直接选边

## 5. 纠偏与 task_id

任何 Task 返回首个非空回执后，旧 `task_id` 立即失效，不得再次传给任何 agent。状态只描述本次结果，不授予恢复权。

唯一例外是 `task_result` 缺失、空串或纯空白，且没有形成任何可用回执：

1. 仅在同一主会话、同一任务单、同一 agent 与同一范围内复用原 `task_id`，提示其继续并输出缺失结果。
2. 写盘 agent 空返回时先核对实时 Git、任务单结果和验证现场；若实施或提交已发生，只允许补写结果/回执，不得重复写盘、验证或提交。
3. 一旦取得非空结果、明确失败/取消、任务单或用户要求变化、出现直接冲突的外部工作区漂移，或跨越会话/交接边界，原 ID 立即失效。
4. 连续空返回不设固定尝试数；可继续索取回执，但出现无新信息循环时停止并诊断或创建全新 task。空返回 ID 不写入 handoff。
...
- reviewer 仅在任务单明确要求复验时使用 Gradle 协议；运行态交用户，双基线脚本不授权
...
- 涉及 docs 改动后或合并前运行文档纪律门禁；涉及 src 类删除、新增或改包路径时运行文档事实漂移门禁

## 7. 用户边界与跨会话

产品行为或优先级、公共 API/兼容性/数据格式、宪章偏离、显著残余风险，以及 merge、push、tag、release、删除、生产、auth、密钥等事项由用户决定。问题使用中文集中提出，subagent 不替用户拍板。

跨会话工作记忆见 [`SESSION-HANDOFF.md`](SESSION-HANDOFF.md)。新会话先核对任务单、Git 与关键事实，再自动续接；不得从 handoff 复用任何 `task_id`，空回执例外只限产生它的当前主会话。
