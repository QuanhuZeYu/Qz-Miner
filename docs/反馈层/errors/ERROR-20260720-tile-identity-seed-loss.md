...
同类缺口后来出现在范围交互：原右键回调没有携带 seed，下一 tick 规划才回读 world。若原右键已收获作物、改变 metadata、吸取液体或令方块变为空气，规划拿到的是动作后的状态，目标集合与玩家实际触发对象不一致。后续还确认 `RIGHT_CLICK_AIR` 只描述 Forge 动作分支，其占位坐标不是语义目标；必须先在同一回调内用当前射线解析 BLOCK hit。
...
范围交互路径同样经 `RightClickObserved → PlanStarted` 延迟消费；不同点是原右键回调发生在动作生效前，因此该回调就是语义目标 block、完整 metadata 与 live TileEntity 的最后存活窗口。BLOCK 可直接提供目标坐标，AIR 必须先以当前 side-neutral 射线解析；把“Forge action 自带目标”或“右键块通常仍在 world”当成保证，会在桶、收获和模组交互上稳定丢失或错认 seed。
...
- 在服务端原右键回调中先解析语义目标：非液体 BLOCK 保留 event 坐标，AIR 从当前射线取得 BLOCK hit，液体 BLOCK/AIR 都使用包含液体的射线；AIR 占位坐标永不进入 seed。随后在 publish 前冻结 `Block`、完整非负 int metadata 与 live TileEntity token；状态机原样传播，规划桥优先使用冻结三元组。读取异常固定为 `UNRESOLVED`，不得下一 tick 用后来 world 值覆盖或猜测。
...
- 为 token 纯值约束、完整身份矩阵、adapter 失败不降级、未知同/异类、无 TileEntity、破坏/右键事件原样传播、BLOCK/event 与 AIR/ray 目标来源、完整 int metadata、legacy `UNRESOLVED` 与 worker 启动前取消建立纯 JVM/结构回归。
