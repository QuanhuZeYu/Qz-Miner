...
- 5.2 branch CI 由 runner 声明 `VERSION=5.2.1-ci+${{ github.sha }}`，并检查 generated `Tags.VERSION` 与主 JAR `Tags.class` 的 exact 值；两个 matrix leg 对同一 SHA 使用同一版本。
- tag workflow 继续使用 tag 名注入 `VERSION`。本地未显式注入时固定使用合法的 `5.2.1-dev`，只作
