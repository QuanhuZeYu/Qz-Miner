...
- `client.objectGroups.members` 使用 UILib 4.6.0 `LIST_MEMBERS`：Picker portal 展开当前组的每个成员，编辑按稳定 ID 只替换目标成员，新增则追加，重复 registry 不自动合并。旧 aggregate codec 的同 registry 归一仅作为兼容接口保留，不定义当前 UI 语义；删除和原始值修正仍走 raw 列表。合法未知 selector 展示 canonical，malformed 成员展示通用占位，外层 raw 值保持无损。
