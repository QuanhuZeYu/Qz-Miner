...
- S2C：`PacketChainConfigSync` Handler 在 Netty 线程只转交三个原始 int；`ClientProxy` 捕获 `ClientConnectionLifecycle` token 后经 `ClientChainConfigSyncDispatch` 投递。inactive token 直接丢弃且不做 dispatcher rejection warn；排队任务在客户端主线程整包校验后、publication 前要求 captured token 仍为 current 且 active。connect 推进 active、disconnect 推进 inactive、client world unload 推进 token 但保持当前 active 标志（切维度后同连接仍可捕获新 active token）。dispatcher 拒绝时 `RateLimitedRejectDiagnostics` 仅 CAS 获胜线程限频汇总，不跨 lifecycle 重试。客户端主线程整包要求 radius/maxBlocks 大于零且 matchedCount 非负，非法包保留旧三字段；**不**检查 matchedCount≤maxBlocks（规划启动后配置可能下调）。LOGIN 的 matchedCount=0 合法。
