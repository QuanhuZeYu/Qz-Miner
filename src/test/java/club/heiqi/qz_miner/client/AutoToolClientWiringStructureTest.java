...
public class AutoToolClientWiringStructureTest {
    ...
    public void proxyInstallsOneAdapterAndConfigAndLifecycleNotifyIt() throws Exception {
        ...
        Assert.assertFalse(proxy.contains("handleClientAutoToolSwapTakeoverRequest"));
        Assert.assertFalse(proxy.contains("autoToolSwapAdapter.onTakeoverRequest"));
        Assert.assertTrue(proxy.contains("AUTO_TOOL_SWAP_LIFECYCLE_GATE"));
        Assert.assertFalse(proxy.contains("PreviewInvalidationListener"));
        Assert.assertFalse(proxy.contains("chainPreviewController.onToolLayoutVerified"));

        String preview = source("src/main/java/club/heiqi/qz_miner/chain/client/ChainPreviewController.java");
        Assert.assertFalse(preview.contains("public void onToolLayoutVerified"));
