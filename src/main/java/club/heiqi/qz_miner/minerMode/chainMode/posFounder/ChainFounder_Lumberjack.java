...
public class ChainFounder_Lumberjack extends PositionFounder {
    ...
    public ChainFounder_Lumberjack(AbstractMode mode) {
        super(mode);
        nextChainSet.add(center);
        visitedChainSet.add(center);
        int[] sampleOreIDs = OreDictionary.getOreIDs(new ItemStack(mode.blockSample));
    ...
    public boolean filter(Vector3i pos) {
        Block block = manager.world.getBlock(pos.x, pos.y, pos.z);
