...
public class CommonProxy {
    ...
    /**
     * FML 加载完成事件转发（common 侧 no-op；客户端由 {@code ClientProxy} 覆写）。
     *
     * <p><b>通道约定</b>：FML 生命周期事件（{@link FMLLoadCompleteEvent} / {@link FMLModIdMappingEvent}）
     * <b>不得</b>标 {@code @SubscribeEvent}——它们不是 {@code cpw.mods.fml.common.eventhandler.Event}
     * 的子类，注册进 EventBus 会在装配期抛 {@code IllegalArgumentException} 并让客户端启动崩溃。
     * 唯一正确通道：{@code @Mod} 类上的 {@code @Mod.EventHandler} → SidedProxy（本方法）→ 侧实现。</p>
     *
