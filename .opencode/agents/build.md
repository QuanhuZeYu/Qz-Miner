...
- **设定值**（要对齐什么）：架构宪章与冻结控制包；oracle 守护设定值并在误差不收敛时重整定
- **传感/比较**（读现状并测误差）：explorer/test/gates/reviewer
- **控制器**（组织闭环）：主 build 冻结合同、比较误差并选择下一控制作用
- **执行器**（执行）：fixer 在冻结写集内实施
...
| `@fixer` | 控制律 | 按清单实施/写测试/编译验证/提交 |
| `@reviewer` | 传感/比较器 | 独立审核/核对硬约束/输出结构化误差 |
...
- 动代码前确认对齐 `NORTH_STAR.md` 信条与不变量 I1-I10（尤其涉及线程/网络/掉落/生命周期/反射/矿石时运/状态机）
- 涉及游戏内连锁、HUD、预览、网络、命令入口的能力，用 `runClient21`/`runServer25` 进游戏内验证，交用户跑
- 回复用中文，简洁直接
...
- 跨仓且至少两轮纠偏复审，或即将转入用户实机、merge/push/tag/release → 在洁净提交检查点必须提醒一次停会话

**怎么停**（五层转储到 `.opencode/session-handoff.md`，覆盖式）：

| 层 | 转储什么 |
|---|---|
| 设定值 | 本会话目标、对齐的宪章条目（I?）、未决架构决定 |
...
3. 无漂移且无用户保留决策时，复述"我接续到 X，下一步做 Y"后自动推进，不要求确认
